...
class MessageService
{
    ...
    public function getUserConversations(Users $user): array
    {
        ...
        foreach ($messages as $message) {
            ...
            $otherUser = ($message->getSender()->getId() === $user->getId()) ? $message->getRecipient() : $message->getSender();
            ...
            // Increment unread count if the message is for the current user and is not read
            if (!$message->isRead() && $message->getRecipient() && $message->getRecipient()->getId() === $user->getId()) {
        ...
        uasort($conversations, function($a, $b) {
            return $b['lastMessage']->getSentAt() <=> $a['lastMessage']->getSentAt();
        });
    ...
    public function sendMessage(Users $sender, Users $recipient, string $content): void
    {
    ...
    public function markMessagesAsRead(Users $user1, Users $user2): void
    {
        $this->em->getRepository(Messages::class)->createQueryBuilder('m')
            ->update()
            ->set('m.isRead', 'true')
            ->where('m.recipient = :user1 AND m.sender = :user2')
